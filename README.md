...
When mapping or buffering fails inside **`ReplaytSpanExporter.export`**, the exporter returns **`SpanExportResult.FAILURE`** and does not raise into the OpenTelemetry SDK. If the optional **`on_export_commit`** hook **raises**, **`export`** returns **`FAILURE`** with no partial append; a policy **`"deny"`** return is **`SUCCESS`** with no append (see **Optional approval hook** above). Operational detail is written with **`logging`**: loggers live under the **`replayt_otel_span_exporter`** hierarchy (for example **`replayt_otel_span_exporter.exporter`**). Failure records use **ERROR** and include exception info for stack traces. Full prepared attribute maps are not attached to those records; sensitive attribute **keys** follow the same policy as **`replayt_otel_span_exporter.redaction.attribute_key_is_sensitive`**, so values for those keys are not copied into log text or string **`extra`** fields. **Exception text** and **span names** in the primary message and structured fields are **truncated** (at most **1024** and **256** Unicode code points respectively) and **C0 / C1 control characters** are replaced with spaces so log pipelines see bounded, line-safe fields. Treat exception text as **untrusted** for security policy even after that sanitization; tracebacks from **`exc_info`** can still echo the raw exception line. Field names, numeric limits (**§5.4**), batch semantics, and redaction rules are defined in **[docs/SPEC_SPAN_EXPORT_FAILURE_HANDLING.md](docs/SPEC_SPAN_EXPORT_FAILURE_HANDLING.md)**.
