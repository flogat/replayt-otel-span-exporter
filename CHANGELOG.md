...
- **Beta / 1.0 roadmap and deprecation (spec):** **[docs/SPEC_BETA_AND_STABLE_PROMOTION.md](docs/SPEC_BETA_AND_STABLE_PROMOTION.md)** — Mission Control **`e9115fe4-6d8f-44b7-97e1-008a1a8cf478`**, phase **2** (**Spec lead**): backlog **“Post-alpha roadmap: beta or 1.0 criteria and deprecation policy”** — new **§6** deprecation policy (what **MUST** be deprecated, **CHANGELOG** / **`DeprecationWarning`** / spec alignment, timeline), **§7.1** testable documentation closure criteria for this backlog; prior §6–§7 renumbered to §7–§8. **[docs/MISSION.md](docs/MISSION.md)** — release-phase paragraph, scope row, success criteria, spec-to-suite bullet, traceability table + dedicated MC subsection. **[docs/ROADMAP.md](docs/ROADMAP.md)** — stable-row deprecation note, backlog table row. **[docs/CI_SPEC.md](docs/CI_SPEC.md)** §5 — promotion-doc **`pytest`** contract. **[docs/DESIGN_PRINCIPLES.md](docs/DESIGN_PRINCIPLES.md)** — Releases paragraph cites **§6**.

- **Post-alpha roadmap (integrator cross-links):** root **README** overview, **[docs/SPEC_OTEL_EXPORTER_SKELETON.md](docs/SPEC_OTEL_EXPORTER_SKELETON.md)** stability paragraph, and **[docs/MISSION.md](docs/MISSION.md)** Maintainers row now name **deprecation** with beta/stable policy (**[docs/SPEC_BETA_AND_STABLE_PROMOTION.md](docs/SPEC_BETA_AND_STABLE_PROMOTION.md)**). Mission Control **`e9115fe4-6d8f-44b7-97e1-008a1a8cf478`**, phase **5** (**Architect**): backlog **“Post-alpha roadmap: beta or 1.0 criteria and deprecation policy”**.

- **Runnable `scripts/` demo (spec):** **[docs/SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md](docs/SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md)** — Mission Control **`21487c24-8d58-4085-896c-4a6bad8d0af4`**, phase **2** (**Spec lead**): backlog **“Ship a minimal runnable example package or scripts/ recipe”** — aligned opening and traceability; **“example package”** disambiguated (in-repo **`scripts/`** recipe vs separate distribution); **MC normative acceptance criteria** list (Spec gate / Builder). **[docs/MISSION.md](docs/MISSION.md)** — traceability subsection. **[docs/CI_SPEC.md](docs/CI_SPEC.md)** §5 — MC id + spec §6–§8 cross-link. **[docs/ROADMAP.md](docs/ROADMAP.md)** — backlog table row.

- **`scripts/README.md`:** **Tracing setup** paragraph — **`trace.get_tracer(..., tracer_provider=provider)`** vs global **`set_tracer_provider`**, and contrast with the root README quick start / **`tests/readme_usage_example_snippet.py`**. Mission Control **`21487c24-8d58-4085-896c-4a6bad8d0af4`**, phase **5** (**Architect**): backlog **“Ship a minimal runnable example package or scripts/ recipe”**.
...
- **`tests/test_beta_stable_promotion_docs.py`:** asserts **`docs/SPEC_BETA_AND_STABLE_PROMOTION.md`** keeps the **§6** deprecation heading alongside §2–§5; **[docs/CI_SPEC.md](docs/CI_SPEC.md)** §5, **[docs/MISSION.md](docs/MISSION.md)** (spec-to-suite), and the spec intro describe **§2–§6** mechanical coverage vs **§7–§8** review. Mission Control **`e9115fe4-6d8f-44b7-97e1-008a1a8cf478`**, phase **3** (**Builder**): backlog **“Post-alpha roadmap: beta or 1.0 criteria and deprecation policy”**.

- **`tests/test_scripts_otel_prepared_demo.py`:** subprocess run of **`python scripts/otel_to_prepared_demo.py`** asserts exit **0** and labeled stdout for every **`PreparedSpanRecord`** field in **[docs/SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md](docs/SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md)** §3.3. Mission Control **`21487c24-8d58-4085-896c-4a6bad8d0af4`**, phase **3** (**Builder**): backlog **“Ship a minimal runnable example package or scripts/ recipe”**.
...
- **[docs/SPEC_OTEL_EXPORTER_SKELETON.md](docs/SPEC_OTEL_EXPORTER_SKELETON.md)** §2.3 — failure UX “done” bar now cites **`SPEC_SPAN_EXPORT_FAILURE_HANDLING`** **§9** alongside **§7** (architecture review, MC **`dad27282-2904-4839-ba1c-070e8e3ba7c8`**, phase **5**).
...
- **`scripts/otel_to_prepared_demo.py`:** binds the demo with **`trace.get_tracer(..., tracer_provider=provider)`** instead of **`set_tracer_provider`** plus **`finally`** restore, so a normal run does not emit OpenTelemetry’s stderr line about overriding the global tracer provider (the API allows only one successful global set per process). **[docs/SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md](docs/SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md)** §3.1 and MC acceptance item **2** updated accordingly. Mission Control **`21487c24-8d58-4085-896c-4a6bad8d0af4`**, phase **3** (**Builder**): backlog **“Ship a minimal runnable example package or scripts/ recipe”**.
