...
- **Recommended** build/upload steps (Builder or maintainer implements or runs):
  ...
  - Upload with **`twine upload`** (or host-specific upload) using **credentials stored outside the repo** (environment variables, OIDC / trusted publishing, or secret store).
...
2. **Import and version:** Running Python code equivalent to:
   ...
   MUST succeed without installing **`[dev]`** extras.
3. **Runtime deps:** The install MUST **not** pull **`replayt`** into the environment as a dependency of **`replayt-otel-span-exporter`** (consistent with **`[project].dependencies`** in **`pyproject.toml`**).

Record **where** the package was verified (PyPI project URL or private index name) in release notes or the merge handoff so the next phase knows the source of truth.
...
- [ ] **Sdist and wheel** for that version are uploaded to the chosen index.
- [ ] **§5 Verification** install + import + version + no-**`replayt`**-runtime-dep checks pass.
