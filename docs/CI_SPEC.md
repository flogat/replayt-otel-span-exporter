...
**Optional reference docs tree:** **`docs/reference-documentation/`** (when present) is specified in **[docs/SPEC_REFERENCE_DOCUMENTATION.md](SPEC_REFERENCE_DOCUMENTATION.md)**. Default **`pytest`** does **not** collect tests against that directory solely for that spec; a future backlog that adds link or doc checks MUST update this §5 in the same change set.
