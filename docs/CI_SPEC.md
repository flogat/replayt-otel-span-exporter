...
2. **Workflow present:** **`.github/workflows/release.yml`** exists on the default branch and defines a **publish** (or equivalently named) job that performs the steps in [§8.2](#82-build-check-and-upload-steps).
...
10. **Contract tests:** **`tests/test_release_workflow_contract.py`** asserts that **`docs/CI_SPEC.md`** keeps the §8 heading structure documented in that module and that **`release.yml`** contains the triggers, permissions, environment name, **`python -m build`**, **`twine check`**, **`gh-action-pypi-publish`** pin, and **no** **`pip install -e ".[dev]"`**. Extend that test when new normative requirements are added to §8.
