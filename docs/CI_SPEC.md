...
**Beta / stable promotion docs:** **`tests/test_beta_stable_promotion_docs.py`** checks that **`docs/SPEC_BETA_AND_STABLE_PROMOTION.md`** keeps the normative §2–§5 headings (promotion gates, semver tables, changelog discipline, optional hooks) and that **`docs/ROADMAP.md`** still points integrators and maintainers at **MISSION**, **SPEC_FIRST_ALPHA_RELEASE**, and the promotion spec. **§6** (deprecation policy) and **§7–§8** (acceptance checklist / non-goals) are **normative** but **not** mechanically asserted by that test—Spec gate / maintainer review covers them. If that test module name changes, update this sentence in the same maintenance pass.
