...
- **Spec-to-suite mapping (what CI is expected to prove):**
  ...
  - **[SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md](SPEC_SCRIPTS_OTEL_PREPARED_DEMO.md)** — runnable **`scripts/otel_to_prepared_demo.py`**, documented invocation, and CI proof via **`tests/test_scripts_otel_prepared_demo.py`** (name recorded in **[CI_SPEC.md](CI_SPEC.md)** §5 once the Builder adds it).
