...
6. Default CI (**[docs/CI_SPEC.md](CI_SPEC.md)**) runs **`pytest`** in a way that collects the new tests on every **Python** version in the **`ci.yml`** matrix (**3.11** and **3.12** today; **3.13** as well when [§3.1](CI_SPEC.md#31-python-313-matrix-expansion-normative-backlog) is satisfied) without extra flags.
