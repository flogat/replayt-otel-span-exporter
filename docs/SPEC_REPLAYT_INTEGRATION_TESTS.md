...
- **Data passed across the boundary:** A plain **`dict`** built from **`PreparedSpanRecord`** (**`trace_id`**, **`span_id`**, **`name`**, **`kind`**, **`start_time_unix_nano`**, **`end_time_unix_nano`**, **`attributes`**) is written with **`RunContext.set("otel_span", ...)`** inside **`Runner`**’s **`before_step`** callback; the step handler reads it with **`get`** and asserts equality. When **[docs/SPEC_EXPORT_TRIAGE_METADATA.md](SPEC_EXPORT_TRIAGE_METADATA.md)** is implemented, the same payload **SHOULD** include **`workflow_id`** and **`step_id`** (copied from the record, which may be **`None`**) so replayt hooks see triage metadata without reading attribute keys alone; update **`tests/integration/test_replayt_boundary.py`** in the same change.
